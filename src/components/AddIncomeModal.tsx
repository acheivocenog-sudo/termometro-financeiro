...
export default function AddIncomeModal({ open, onClose, onSaved }: AddIncomeModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Store at noon UTC so the date never shifts across midnight regardless of timezone
    const res = await fetch('/api/incomes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...form, amount: parseFloat(form.amount), date: `${form.date}T12:00:00.000Z` }),
    })
